...
def _detect_affected_systems(question: str, snapshot: Dict[str, Any]) -> List[str]:
    ...
    keyword_map = [
        ...
        (
            ...
            ["communication", "communications", "signal", "satcom"],
        ),
    ...
    top_names = " ".join(
        item["object_name"].lower() for item in snapshot.get("top_objects", [])
    )
    if "starlink" in top_names and "Starlink" not in systems:
        systems.append("Starlink")
    if (
        any(
            "iss" in item["object_name"].lower()
            for item in snapshot.get("top_objects", [])
        )
        and "ISS" not in systems
    ):
        systems.append("ISS")
    return systems[:5] or ["Satellite Communications", "Launch Windows"]


def compute_risk_relevance(
    question: str, snapshot: Dict[str, Any], include_live_odri: bool
) -> float:
...
def _summarize_top_objects(objects: Iterable[Dict[str, Any]]) -> str:
    ...
    return (
        "; ".join(parts)
        if parts
        else "No elevated objects were found in the current cache."
    )
...
def _fallback_answer(
    question: str, snapshot: Dict[str, Any], affected_systems: List[str]
) -> str:
    top_objects = snapshot.get("top_objects", [])
    summary = snapshot.get("summary", {})
...
async def generate_cascade_response(
    question: str,
    snapshot: Dict[str, Any],
    include_live_odri: bool,
    ...
    answer = await _groq_answer(question, context, system_prompt)
    if not answer:
