...
def _build_timelines(cdms: list) -> dict:
    ...
    for cdm in cdms:
        ...
        if pair_key not in timelines:
            timelines[pair_key] = {
                "sat1": sat1, "sat2": sat2, "cdms": [],
                "miss_distance_trend": "unknown", "pc_trend": "unknown",
                "max_pc": 0, "min_miss_distance_km": float("inf"),
            }
...
async def get_cdm(
    ...
    try:
        ...
        print(f"[CDM] Error: {exc}")
        return make_json_safe({
            "source": "unavailable",
            "count": 0, "original_count": 0,
            "conjunctions": [], "error": str(exc),
        })
...
async def refresh_cdm():
    try:
        ...
        return make_json_safe({"status": "refreshed", "count": len(unique), "original_count": len(cdms)})
    except Exception as exc:
        print(f"[CDM/REFRESH] Error: {exc}")
        return make_json_safe({"status": "error", "count": 0, "original_count": 0})


@router.get("/cdm/timeline")
async def cdm_timeline(
    sat1: Optional[str] = Query(default=None),
    sat2: Optional[str] = Query(default=None),
    min_pc: float = Query(default=1e-6),
    ...
    try:
        ...
        if sat1 or sat2:
            ...
            for key, data in timelines.items():
                name1, name2 = data["sat1"].upper(), data["sat2"].upper()
                match = False
                if sat1 and sat2:
                    match = (sat1_upper in name1 and sat2_upper in name2) or (sat1_upper in name2 and sat2_upper in name1)
                elif sat1:
                    match = sat1_upper in name1 or sat1_upper in name2
                if match:
                    filtered_timelines[key] = data
            timelines = filtered_timelines
        sorted_timelines = sorted(timelines.values(), key=lambda x: x.get("max_pc", 0), reverse=True)
        return make_json_safe({
            "source": "Space-Track.org — 18th Space Defense Squadron",
        ...
        print(f"[CDM/TIMELINE] Error: {exc}")
        return make_json_safe({"timeline_count": 0, "timelines": []})


@router.get("/cdm/highrisk")
async def highrisk_cdm(
    threshold_pc: float = Query(default=1e-4),
    limit: int = Query(default=10, ge=1, le=50),
):
    try:
        cdms = load_cdm_cache()
        unique_cdms = deduplicate_cdms(cdms)
        high_risk = [_extract_cdm_summary(c) for c in unique_cdms if _safe_float(c.get("P_C",0)) >= threshold_pc]
        high_risk.sort(key=lambda x: x["probability_of_collision"], reverse=True)
        return make_json_safe({
            "source": "Space-Track.org — 18th Space Defense Squadron",
            "description": f"Conjunctions with Pc >= {threshold_pc}",
            "count": len(high_risk), "threshold_pc": threshold_pc,
            "conjunctions": high_risk[:limit],
        })
    except Exception as exc:
        print(f"[CDM/HIGHRISK] Error: {exc}")
        return make_json_safe({"count": 0, "conjunctions": []})
